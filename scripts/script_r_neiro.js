...
const poyasnenie = document.getElementById('poyasnenie'); // Добавляем объявление элемента
...
function drawSymbolr(progressr) {
    ...
    ctxr.fillStyle = "#ccc";
    // Контур символа
    ctxr.fillRect(50, 50, 100, 20);
    ctxr.fillRect(130, 50, 20, 50);
    ctxr.fillRect(50, 90, 100, 20);
    ctxr.fillRect(50, 90, 20, 50);
    ctxr.fillRect(50, 130, 100, 20);
...
function animateFillr() {
    ...
    if (progressr >= 1) {
        ...
        progressr = 0;
        stager++;
        if (stager > totalStagesr) {
            return; // Останавливаем анимацию после последнего этапа
        }
    }
...
r.addEventListener('mouseenter', () => {
    // Сбрасываем прогресс и этапы при каждом наведении
    filledStagesr = [false, false, false, false, false];
    progressr = 0;
    stager = 1;
    animateFillr();
    poyasnenie.innerHTML = 'фонема л/р<br> корейский л, чуть мягче, чем русский л<br>корейский р - в отличие от русского - не дрожащий, одноударен и более мягкий';
});
